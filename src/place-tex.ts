...
export function placeTex(group, x, y, tex, options:placeTexOptions = defaultOptions ) {
    ...
    group.setAttribute('transform', `translate(${x - bbox.width/2}, ${y - bbox.height/2}) scale(${scale})`);

    if (options.background) {
        let margin = 8;
        let groupBbox = group.getBoundingBox()
        let rectangle = new Rectangle(groupBbox.x - margin / 2, groupBbox.y - margin / 2, groupBbox.width + margin, groupBbox.height + margin);
